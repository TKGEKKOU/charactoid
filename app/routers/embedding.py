...
@router.patch("/config")
def configure(payload: EmbeddingResourceConfig, request: Request, x_charactoid_request: str = Header(default="")):
...
def install(payload: EmbeddingResourceConfig, request: Request, x_charactoid_request: str = Header(default="")):
    protected(request, x_charactoid_request)
    try:
