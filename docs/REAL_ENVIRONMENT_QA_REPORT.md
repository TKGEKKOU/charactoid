...
- `.pth` 音色文件和 `.index` 索引文件都支持在对话卡片中直接导入。
- 两类文件均可通过“打开文件夹”查看受管音色目录。
- 打开文件夹与导入操作使用紧凑的同一行按钮，不改变 RVC 主流程。
- `.index` 仍然是可选项；不导入或不选择 Index 也可以继续变声。

### TTS 与 GPT-SoVITS

- 本地 GPT-SoVITS 使用项目内置适配器调用其 HTTP `/tts` 接口，并通过角色绑定的 Voice Asset 提供参考音频、模型和语言参数。
- OpenAI-compatible TTS 与 GPT-SoVITS 是两条不同的实现路径：前者调用 `/audio/speech`，后者调用本地 GPT-SoVITS 服务；不能把任意 TTS API 直接宣称为 GPT-SoVITS 服务。
- 因此，当前可以确认“项目的 TTS API 能接入 GPT-SoVITS 本地服务”，但不能保证第三方 TTS API 具备 GPT-SoVITS 的模型和音色能力。

### Reranker

- 本地 Reranker 由受管模型资源提供；百炼 Reranker 通过兼容 `/reranks` 或原生文本重排接口适配。
- RAG 运行时会把不同响应格式转换为统一的候选分数合同，再交给检索图精排。
- 连接测试只验证服务可达和响应结构；真实排序质量仍需要使用实际知识库查询验证。

### 资源卡片说明

对话中的 Embedding、Reranker、RVC、GPT-SoVITS、ASR、人声分离和 FFmpeg 均按“可选、按需准备”展示。未准备某项资源不等于应用整体故障；只有使用对应功能时，才需要安装、配置或启动该资源。
