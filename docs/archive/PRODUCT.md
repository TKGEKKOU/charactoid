...
web（本地优先：FastAPI 服务 + 浏览器工作台，`http://127.0.0.1:18000/static/index.html`；可选 pywebview 宿主窗口仅用于启动进度）
...
CHARACTOID 是本地优先的角色对话应用。声音工坊把"素材 → GPT-SoVITS 音色"的全流程可视化：从视频/音频提取人声、切片、校验转写、训练模型，并供角色编辑页绑定。
