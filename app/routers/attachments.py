...
async def upload_attachments(conversation_id: str, request: Request, files: list[UploadFile] = File(...), x_charactoid_request: str = Header(default=""), session: Session = Depends(get_session)):
    guard(x_charactoid_request)
    result=[]
    created_items = []
    try:
        for upload in files:
...
def remove_attachment(conversation_id: str, file_id: str, request: Request, x_charactoid_request: str = Header(default=""), session: Session = Depends(get_session)):
    guard(x_charactoid_request); item=get_item(session,request,conversation_id,file_id); delete_attachment(session,request.app.state.settings.project_root,item); session.commit(); return None

@router.post("/api/conversations/{conversation_id}/attachments/{file_id}/send-to-rvc")
def send_to_rvc(conversation_id: str, file_id: str, request: Request, x_charactoid_request: str = Header(default=""), session: Session = Depends(get_session)):
    guard(x_charactoid_request); item=get_item(session,request,conversation_id,file_id)
...
def send_to_rag(conversation_id: str, file_id: str, request: Request, x_charactoid_request: str = Header(default=""), session: Session = Depends(get_session)):
    guard(x_charactoid_request); item=get_item(session,request,conversation_id,file_id)
